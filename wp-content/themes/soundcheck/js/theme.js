		...
		$('[id^="jp_container_"]').each(function() {
	    	...
	    	thePlaylist = new jPlayerPlaylist({
				...
				ready: function() {
				    on_ready();
				    add_track_number();
				    console.log(templateDirectoryUri);
				},
	    		play: function() {
	    			on_play();
	    		},
			...
			function show_hide_playlist(shouldShow) {
				if( thePlayer.parent().parent().hasClass('widget') ) {
					if(shouldShow){
						thePlayer.addClass( 'playlist-open');
						jpPlaylistNav.fadeIn('fast');
					} else {
						jpPlaylistNav.fadeOut('fast', function() {
							thePlayer.removeClass( 'playlist-open');
						});
        			}
				} else {
					if( shouldShow ) {
...
/*
/*	This is primarly to detect if two audio players with the same ID are on the page.
/*	If so, we'll remove the duplicate audio players that are not in the main content.
/*-----------------------------------------------------------------------------------
	$('[id]').each(function(){
		var ids = $('[id="'+this.id+'"]');
		if(ids.length > 1 && ids[0] == this) {
			console.warn('Multiple IDs #'+this.id+'. Duplicates hidden via JS (js/theme.js)');
			duplicateAudio = '.audio-player #'+this.id;
			$(duplicateAudio, '.sidebar, #hero').remove();
		}
	});
*/
});
